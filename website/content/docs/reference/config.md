...
| `sidebar`        | `{ collapse: boolean }`             | `{ collapse: true }`          | Sidebar folder behaviour. `collapse: true` (default) renders each folder as a collapsible disclosure, closed by default — the branch containing the current page always stays open, so you can see where you are. Set `collapse: false` to render the whole tree auto-expanded. |
