...
`site.css` や `palette: 'bare'` では足りないとき — 重ねるのではなく Ovellum の
スタイルシートとクライアントスクリプトを**丸ごと置き換えたい**とき —
[`site.templateDir`](/ja/docs/reference/config/) がそのアセットを引き継ぐディレクトリを指します。

```ts
site: {
  templateDir: './theme',   // プロジェクトルートからの相対
}
```

```text
theme/
  style.css     → /assets/ovellum.css として出力（バンドルのテーマ CSS を置き換え）
  script.js     → /assets/ovellum.js として出力（バンドルのランタイムを置き換え）
  fonts/        → /assets/fonts/ として出力（あなたのウェブフォント）
```

**ファイル単位**のフォールバック付きオーバーライドです。`style.css` だけ用意すれば、
あなたの CSS + Ovellum のバンドル `script.js` になります。`script.js` だけ用意すれば、
デフォルトのテーマ CSS が残ります。気にするレイヤーだけを引き継げます。

**何をカバーし、何をしないか。** これは*スタイリングとクライアント挙動*を置き換えます —
`style.css` がスタイルシートそのものになるので、[生成される HTML](#今日バンドルされているもの-vs-計画中のもの)
が使う同じ `ov-*` クラス名（トップバー、サイドバー、外観パネル、コンテンツカードなど）を
対象にします。ページの **HTML 構造はコードで生成され**、ここで上書きするテンプレートでは
ありません — レイアウト／パーシャルのシステムは対象外です。一から書き直すのではなく色・
フォントの調整なら、まず `site.css` か `palette: 'bare'` を使ってください。`templateDir` は
パッケージをフォークせずに完全な制御が欲しいときの避難ハッチです。
...
- ページの **HTML 構造**を上書きするためのレイアウト／パーシャルのシステム（今日はマークアップがコードで生成されます）。

CSS/JS レイヤーは今日すでに完全にカスタマイズできます。[`site.css`](#デフォルトテーマのカスタマイズ) はオーバーライドを重ね、[`palette: 'bare'`](#bare-モード) は色をすべてホストに委ね、[`site.templateDir`](#テンプレートディレクトリを持ち込む) はバンドルのスタイルシート + スクリプトを丸ごと置き換えます — いずれもパッケージのフォークは不要です。上記のロードマップ項目は CSS/JS を*超えて*、生成されるマークアップそのものを変えることに関するものです。それが意図的な v1 の制約です — スタイリングの表面は開かれており、構造はまだコードです。
