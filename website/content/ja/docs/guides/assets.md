...
再圧縮よりさらに踏み込むには、`format: 'webp'` を設定してラスター画像を
[WebP](https://developer.mozilla.org/docs/Web/Media/Formats/Image_types#webp)
に変換します（PNG/JPEG よりはるかに小さく、ブラウザ対応は約 97%）:
...
`.png` / `.jpg` / `.jpeg` アセットは隣に `.webp` として書き出され、Ovellum は
...
`og:title`、`og:url`、`twitter:card` のメタが追加されます。ドラフトと 404 ページは
