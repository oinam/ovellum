...
## 作成者が制御する HTML（`site.headExtra`）

[`site.headExtra`](/ja/docs/reference/config/#site) は、各ページの `<head>` に
**そのまま** 注入されます — エスケープもサニタイズもされません — ので、解析スニペットや
同様のサードパーティ製 `<script>`/`<link>`/`<meta>` タグが書いたとおりに動作します。
これは設計上の **信頼境界** です。すべてのページで任意のスクリプトを実行できます。

これはサーバー設定と同じように扱ってください — サイトの管理者/作成者だけが設定すべきで、
自分が制御するリテラル文字列だけにしてください。`headExtra` を信頼できない入力（閲覧者が
提供したコンテンツ、リクエストデータ、CMS のフィールド、自分が所有しない環境変数）から
**決して** 導出しないでください。そうすると、サニタイザーが防いでいるはずのインジェクションを
そのまま再導入してしまいます。デフォルトでは未設定なので、エンドユーザー向けドキュメントは
何も注入されない状態で出荷されます。
...
- **サプライチェーンのピン留め。** `package.json` はキャレット範囲（npm の
  ...
  見直す価値があります。
- **`input/` 内のシンボリックリンクされた Markdown。** パススルーアセットは
  保護されています――ビルドは、シンボリックリンク経由でコンテンツディレクトリの
  外に解決されるファイルをスキップします――が、`input/` 内のシンボリックリンク
  された _Markdown_ ファイルは依然として読み込まれ、レンダリングされます。緩和策は
  コンテンツディレクトリを信頼することであり、このケースは検出しようとはしません。
