...
sourceHash: '920219b2301eefa4'
---

# アンカーと保護ゾーン

ハイブリッドモードが機能するのは、Ovellum と書き手が2種類のマーカーについて
合意しているからです。この2つが合わさって一つの取り決めを形づくります。保護ゾーンの
内側にあるものは書き手のもの、それ以外のすべてはツールのものです。
...
アンカーが持つのは `id` **だけ**です — タイムスタンプも、それ以外の属性もありません。
これは意図的なものです。生成は決定論的なので、変更のないソースを再生成すると
バイト単位で同一の出力になります。ファイルは何も変化しないため、バージョン管理は
差分を検出せず、ページの[「Edited」日付](/ja/docs/reference/config/#pagemeta)は
ビルドのたびに「今日」へリセットされることなくそのまま保たれます。
...
ソースコメントに `@preserve` が現れると、**ハイブリッド**ビルドはそのシンボルの
生成セクションを `@manual` 保護ゾーン（シンボルのアンカー id をキーにする）で自動的に
囲みます。最初のビルドでゾーンに生成内容を種として入れ、それ以降はその内側で編集した
ものが、手書きのゾーンとまったく同じように再生成を生き延びます。シンボルが削除・リネーム
された場合、その文章は失われるのではなく[孤立](/ja/docs/concepts/orphans/)します。

これはハイブリッドモードの機能です。`auto` モードは毎回完全に再生成するため、ゾーンを
出力しません（保持されるものがないからです）。アンカーコメントはゾーンの外側に残るので、
再アタッチと孤立追跡は引き続き機能します。

`@preserve` は**シンボル単位**です。クラスに付けた場合は、クラスレベルのドキュメント
ブロックだけを保護します。各メソッドは独立しており、メソッドのセクションが囲まれるのは、
そのメソッド自身が `@preserve` を持つ場合だけです。つまり、手書きのクラスノートが 1 つ
あっても、すべてのメソッドのドキュメントが凍結されることはありません。（テーブルとして
レンダリングされるプロパティは決して囲まれません。）
