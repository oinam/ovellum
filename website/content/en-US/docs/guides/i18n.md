...
Each locale also gets its own RSS feed — `/feed.xml` for the default language,
`/<code>/feed.xml` for the rest — scoped to that locale's pages (the sitemap
stays a single combined file). Requires `site.baseUrl`.
