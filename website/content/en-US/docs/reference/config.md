...
| `appearance`     | `'control' \| 'inherit'` or an inherit object | `'control'`         | Who owns the light/dark switch. `'control'` (default) — Ovellum's appearance panel drives it. `'inherit'` makes the docs **follow a host project** instead: the Mode toggle is removed from the panel, Ovellum stops persisting its own choice, and light/dark resolves from `prefers-color-scheme`. For a host whose toggle is a JS choice in **same-origin `localStorage`** (next-themes, a Tailwind `class` strategy), use `{ mode: 'inherit', storageKey: 'theme', darkValue?: 'dark', lightValue?: 'light' }` — Ovellum reads that key on load and live-updates on cross-tab `storage` events, falling back to `prefers-color-scheme` for `'system'`/unknown values. Pair with [`site.css`](#css) (which inherits the *colors*); `appearance` decides which mode is active. See the [theming guide](/docs/guides/themes/#following-the-hosts-lightdark-switch). |
| `defaultTheme`   | `'auto' \| 'light' \| 'dark'`       | `'auto'`                      | Initial light/dark mode before user preference loads. Visitors can change it from the topbar appearance control (persisted in `localStorage`). When [`appearance: 'inherit'`](#appearance) this is just the no-JS fallback (the host drives the live value).                                                                                  |
| `palette`        | `'default' \| 'nord' \| 'flexoki' \| 'solarized' \| 'eink' \| 'bare'` | `'default'`  | Initial page-wide color palette before user preference loads (`'default'` displays as "Ovellum" in the picker). Every palette ships light **and** dark variants; the mode choice stays independent. Visitors can switch palettes from the topbar appearance control. The special value **`'bare'`** ships **no baked palette** — Ovellum's color + `--font-body` tokens become `var(--ov-host-*, <default>)`, so a host stylesheet (via [`site.css`](#css)) defining the `--ov-host-*` names is the sole source of color (define none → default look). The Theme picker is dropped. The cleanest "drop into a host app and match" path; pairs with [`appearance: 'inherit'`](#appearance). See the [theming guide](/docs/guides/themes/#bare-mode). |
...
| `headExtra`      | `string?`                           | `undefined`                   | Raw HTML injected verbatim into `<head>` on every page, just after the [`css`](#css) stylesheets and before the inline theme-boot script. **Not escaped or sanitized** — only set markup you control. Unset by default. Primary use: analytics snippets, e.g. `'<script defer src="https://analytics.example.com/script.js" data-website-id="…"></script>'`. |
| `css`            | `string \| string[]?`               | `undefined`                   | Extra stylesheet URL(s) linked into `<head>` **after** the base theme CSS, so their rules win the cascade. A single URL or an array; each becomes a `<link rel="stylesheet">`. Relative / root-absolute paths resolve against the site (basePath-aware); `http(s)://` URLs pass through. The supported hook for **theme inheritance / overrides** — point it at a stylesheet that re-declares Ovellum's [design tokens](/docs/guides/themes/#inheriting-a-host-projects-design) (`--color-bg`, `--color-fg`, `--font-body`, …) and the template re-skins to match a host design system. Validated to stylesheet links only (rejects `javascript:` / `data:` schemes), unlike the raw [`headExtra`](#headextra). |
...
| `templateDir`    | `string?`                           | `undefined`                   | Path (relative to the project root) to a custom template directory whose assets **replace** the bundled theme's, per file with fallback: `style.css` → `/assets/ovellum.css`, `script.js` → `/assets/ovellum.js`, `fonts/` → `/assets/fonts/`. Provide only some and the rest fall back to the default. "Bring your own template directory" — full control of the CSS/JS layer without forking. **The page HTML is generated in code**, so this overrides styling + client behavior (your `style.css` targets the same `ov-*` classes), not the markup. For token tweaks prefer [`css`](#css) or [`palette: 'bare'`](#palette). See the [theming guide](/docs/guides/themes/#bring-your-own-template-directory). |
| `images`         | `{ quality?, format?, maxWidth? }?` | `undefined`                   | **Opt-in image optimization.** When set, raster assets (`.jpg`/`.jpeg`/`.png`/`.webp`/`.avif`) are re-encoded **in place** during the build — same path + format, smaller bytes, so `<img src>` never changes. `quality` (1–100, default `80`) applies to lossy formats; PNG is recompressed losslessly. A re-encode that would be larger keeps the original; SVG/GIF pass through. **`format: 'webp' \| 'avif'`** instead *converts* `.png`/`.jpg`/`.jpeg` to a sibling `.webp`/`.avif` and rewrites the matching Markdown `<img src>` to it (not compatible with [`assetBaseUrl`](#assetbaseurl)). **`maxWidth`** downscales any raster wider than that many pixels (aspect kept, never enlarges) - a screenshot-heavy site's one-line diet. Uses the **optional** `sharp` peer dependency (`npm i sharp`) — lazy-loaded only when this is set, so default installs stay lean. See [Assets → Optimizing images](/docs/guides/assets/#optimizing-images). |
| `ogImage`        | `boolean \| { background?, foreground? }?` | `undefined`            | **Opt-in OpenGraph cards.** `true` (or a `{ background, foreground }` object for colors) renders a 1200×630 social-share image per page — title + site name on a flat background — and emits `og:image` / `twitter:image` (+ `og:title`/`og:url`/`twitter:card`) meta. **Requires [`baseUrl`](#baseurl)** (social tags are absolute URLs); set without it, the build warns and generates nothing. Drafts and the 404 page are excluded; the landing page gets a card too. Uses the **optional** `sharp` peer dependency. See [Assets → Social share images](/docs/guides/assets/#social-share-images-opengraph). |
| `minify`         | `boolean?`                          | `false`                       | **Opt-in CSS/JS minification.** When `true`, **author-supplied** `.css`/`.js` are minified during the build — content-folder passthrough files and a custom [`templateDir`](#templatedir)'s `style.css`/`script.js`. The bundled theme is already minified and HTML pages aren't touched, so this only affects your own assets. A minified output larger than the original is discarded; a file that fails to minify is copied as-is with a warning. Uses the **optional** `esbuild` peer dependency (`npm i esbuild`), lazy-loaded only when `true`. See [Assets → Minifying CSS and JS](/docs/guides/assets/#minifying-css-and-js). |
...
| `footerNav`      | `Array<{label, href, icon?, external?}>` | `[]`                     | Links rendered in the footer, in order. Same item shape as [`topbarNav[]`](#topbarnav). |
