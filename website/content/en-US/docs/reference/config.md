...
| `images`         | `{ quality? }?`                     | `undefined`                   | **Opt-in image optimization.** When set, raster assets (`.jpg`/`.jpeg`/`.png`/`.webp`/`.avif`) are re-encoded **in place** during the build — same path + format, smaller bytes, so `<img src>` never changes. `quality` (1–100, default `80`) applies to lossy formats; PNG is recompressed losslessly. A re-encode that would be larger keeps the original; SVG/GIF pass through. Uses the **optional** `sharp` peer dependency (`npm i sharp`) — lazy-loaded only when this is set, so default installs stay lean. See [Assets → Optimizing images](/docs/guides/assets/#optimizing-images). |
