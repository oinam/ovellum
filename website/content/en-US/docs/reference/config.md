...
| `images`         | `{ quality?, format? }?`            | `undefined`                   | **Opt-in image optimization.** When set, raster assets (`.jpg`/`.jpeg`/`.png`/`.webp`/`.avif`) are re-encoded **in place** during the build — same path + format, smaller bytes, so `<img src>` never changes. `quality` (1–100, default `80`) applies to lossy formats; PNG is recompressed losslessly. A re-encode that would be larger keeps the original; SVG/GIF pass through. **`format: 'webp'`** instead *converts* `.png`/`.jpg`/`.jpeg` to a sibling `.webp` and rewrites the matching Markdown `<img src>` to it (not compatible with [`assetBaseUrl`](#assetbaseurl)). Uses the **optional** `sharp` peer dependency (`npm i sharp`) — lazy-loaded only when this is set, so default installs stay lean. See [Assets → Optimizing images](/docs/guides/assets/#optimizing-images). |
| `ogImage`        | `boolean \| { background?, foreground? }?` | `undefined`            | **Opt-in OpenGraph cards.** `true` (or a `{ background, foreground }` object for colors) renders a 1200×630 social-share image per page — title + site name on a flat background — and emits `og:image` / `twitter:image` (+ `og:title`/`og:url`/`twitter:card`) meta. **Requires [`baseUrl`](#baseurl)** (social tags are absolute URLs); set without it, the build warns and generates nothing. Drafts and the 404 page are excluded. Uses the **optional** `sharp` peer dependency. See [Assets → Social share images](/docs/guides/assets/#social-share-images-opengraph). |
