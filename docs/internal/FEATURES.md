...
| Sidebar restores scroll to active link                                             | done     | On load, `script.js` scrolls the (independently-scrollable) sidebar so the active link is centred — full-page navigation otherwise resets the sidebar's `scrollTop` to 0 and loses your place in a long nav. Scrolls only the sidebar (never the page), no-ops when it isn't scrollable (mobile) or the link is already visible. (Fixed 2026-06-13: it read `.offsetHeight` off a DOMRect → `NaN` → never scrolled; now uses `.height`.) |
...
| Frontmatter `permalink` + `tags`                                                   | done     | `permalink` (frontmatter) overrides a page's URL — normalised to root-absolute + trailing slash; the page renders at that path (via the nav URL map) and the nav/links follow. `tags` (string or string[]) → `<meta name="keywords">`. `title` / `description` already respected. (2026-06-13) |
