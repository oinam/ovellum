...
| `version`      | `string?`                     | `undefined`               | template (small chip next to the brand)       | Free-form. Bump on each release. Empty / unset → no badge.        |
| `description`  | `string?`                     | `undefined`               | template (`<meta>`, footer)                   |                                                                   |
| `baseUrl`      | `string?`                     | `undefined`               | template (`<link rel="canonical">`, OG cards, `sitemap.xml`, `feed.xml`) | E.g. `'https://docs.example.com'`. When set, the build emits `sitemap.xml` and an RSS 2.0 `feed.xml` automatically. Omit for relative-link output and no feed. |
...
| `topbarNav`    | `Array<{label, href, external?}>` | `[]`                  | template (right-side nav on every page)       | Items appear in order to the right of the brand. `external: true` (or `href` starting with `http(s)://`) opens in a new tab with `rel="noopener"` and an external-link icon. Below 720px the nav collapses into a hamburger sheet. |
| `footerNav`    | `Array<{label, href, icon?, external?}>` | `[]`             | template (right side of the page footer)      | Right column of the two-column footer (copyright on the left, links on the right). Items with an `icon` field render as icon-only (label still announced to screen readers); items without render as text. Available icon names: `github`, `rss`, `mail`, `package`, `external-link`. `external: true` is auto-detected for `http(s)://` hrefs. |
