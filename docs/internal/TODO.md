...
2. **Branch protection on `main` — deciding later (raised 2026-05-30).** Currently unprotected; maintainer pushes direct to `main`. Recommendation when revisited: turn on the low-friction protections even for a solo repo — **block force-push**, **restrict deletion**, **require the CI status checks** (lint/typecheck/test/build) to pass. The PR-required toggle is the real trade-off: it'd cost `git push` to `main` but would route every change through the changeset-status guard (see gotcha #3). Decide whether the ruleset's bypass list includes the admin (lenient, keeps direct push) or not (strict). Offer stands to generate the ruleset JSON / `gh api` commands.
